...
var MemoizedElement = /*#__PURE__*/React.memo(Element, (prev, next) => {
  return prev.decorate === next.decorate && prev.element === next.element && prev.renderElement === next.renderElement && prev.renderLeaf === next.renderLeaf && isDecoratorRangeListEqual(prev.decorations, next.decorations) && (prev.selection === next.selection || !!prev.selection && !!next.selection && Range.equals(prev.selection, next.selection));
});
/**
...
var Editable = props => {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    var isZeroRect = selectionRect !== undefined && ['x', 'y', 'bottom', 'top', 'left', 'right'].every(prop => selectionRect[prop] === 0); // If the DOM selection is in the editor and the editor selection is already correct, we're done.
    // and we don't have a zero rect

    if (hasDomSelection && hasDomSelectionInEditor && selection && !isZeroRect && Range.equals(ReactEditor.toSlateRange(editor, domSelection), selection)) {
      return;
...
var Slate = props => {
  ...
  var [isFocused, setIsFocused] = useState(ReactEditor.isFocused(editor)); // set the initial is focused state
