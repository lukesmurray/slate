...
var MemoizedLeaf = /*#__PURE__*/React__default['default'].memo(Leaf, function (prev, next) {
  return next.parent === prev.parent && next.isLast === prev.isLast && next.renderLeaf === prev.renderLeaf && next.text === prev.text && next.leaf.text === prev.leaf.text && slate.Text.matches(next.leaf, prev.leaf) && next.leaf[PLACEHOLDER_SYMBOL] === prev.leaf[PLACEHOLDER_SYMBOL];
});
...
var isDecoratorRangeListEqual = function isDecoratorRangeListEqual(list, another) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    var rangeAnchor = range.anchor,
        ...
        rangeOwnProps = objectWithoutProperties(range, ["anchor", "focus"]);

    var otherAnchor = other.anchor,
        otherFocus = other.focus,
        otherOwnProps = objectWithoutProperties(other, ["anchor", "focus"]);
...
var MemoizedElement = /*#__PURE__*/React__default['default'].memo(Element, function (prev, next) {
  return prev.decorate === next.decorate && prev.element === next.element && prev.renderElement === next.renderElement && prev.renderLeaf === next.renderLeaf && isDecoratorRangeListEqual(prev.decorations, next.decorations) && (prev.selection === next.selection || !!prev.selection && !!next.selection && slate.Range.equals(prev.selection, next.selection));
});
/**
...
var Editable = function Editable(props) {
  ...
  useIsomorphicLayoutEffect(function () {
    ...
    var selectionRect = domSelection.rangeCount > 0 ? domSelection.getRangeAt(0).getBoundingClientRect() : undefined;
    var isZeroRect = selectionRect !== undefined && ['x', 'y', 'bottom', 'top', 'left', 'right'].every(function (prop) {
      return selectionRect[prop] === 0;
    }); // If the DOM selection is in the editor and the editor selection is already correct, we're done.
    // and we don't have a zero rect

    if (hasDomSelection && hasDomSelectionInEditor && selection && !isZeroRect && slate.Range.equals(ReactEditor.toSlateRange(editor, domSelection), selection)) {
      return;
...
var Slate = function Slate(props) {
  ...
  var _useState3 = React.useState(ReactEditor.isFocused(editor)),
      ...
      setIsFocused = _useState4[1]; // set the initial is focused state
  ...
  var focusHandler = React.useCallback(function () {
    setIsFocused(ReactEditor.isFocused(editor));
  ...
  }, [focusHandler]); // on blur call the focus handler

  var blurHandler = React.useCallback(function () {
    setIsFocused(ReactEditor.isFocused(editor));
  }, []);
  useIsomorphicLayoutEffect(function () {
